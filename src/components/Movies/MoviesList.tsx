...
export default function MoviesList() {
  ...
  const [searchFilters, setSearchFilters] = useState({ certification: '', genres: '', year: '' });

  if (data.totalPages > 500) { data.totalPages = 500; }
  useEffect(() => {
    const updateData = () => {
      const inputsArray = Object.values(searchFilters).filter(v => v !== '');
      if (inputsArray.length === 1) {
        setUrl(
          `${BASE_URL}discover/movie?${inputsArray[0]}`
        );
        urlRef.current = `${BASE_URL}discover/movie?${inputsArray[0]}`;
      } else if (inputsArray.length === 2) {
        setUrl(
          `${BASE_URL}discover/movie?${inputsArray[0]}&${inputsArray[1]}`
        );
  ...
  function handleFiltersChange(e:{ target: { id: string; value: React.SetStateAction<string>; selectedOptions: HTMLCollectionOf<HTMLOptionElement>};
  }) {
    if (e.target.id === 'searchMoviesByCertification') {
      setSearchFilters(prevState => ({ ...prevState, certification: `certification_country=US&certification=${e.target.value}` }));
    }
    if (e.target.id === 'searchMoviesByGenres') {
      ...
      setSearchFilters(prevState => ({ ...prevState, genres: `with_genres=${genresString}` }));
    }
    if (e.target.id === 'searchMoviesByYear') {
      setSearchFilters(prevState => ({ ...prevState, year: `primary_release_year=${e.target.value}` }));
    }
  }
