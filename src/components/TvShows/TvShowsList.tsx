...
export default function TvShowsList() {
  ...
  useEffect(() => {
    const updateData = () => {
      ...
      if (inputsArray.length === 1) {
        setUrl(
          `${BASE_URL}discover/tv?${inputsArray[0]}`
        );
        urlRef.current = `${BASE_URL}discover/tv?${inputsArray[0]}`;
      } else if (inputsArray.length === 2) {
        setUrl(
          `${BASE_URL}discover/tv?${inputsArray[0]}&${inputsArray[1]}`
        );
  ...
  function handleFiltersChange(e: {
    target: { id: string; value: React.SetStateAction<string>; selectedOptions: HTMLCollectionOf<HTMLOptionElement> };
  }) {
    ...
    if (e.target.id === 'searchTvShowsByYear') {
      setSearchFilters(prevState => ({ ...prevState, year: `first_air_date_year=${e.target.value}` }));
    }
  }
