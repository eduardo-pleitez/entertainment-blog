    ...
    <div>
      {data.results.length > 0 ? (
        <div>
          ...
          <Accordion
            allowToggle
            className='lg:w-9/12 md:w-11/12 w-full mx-auto'
          >
            {data.results.map(
              ({ author, content, createdAt }: PropsReviewElement, index) => {
                return (
                  <AccordionItem key={index}>
                    <h3>
                      <AccordionButton className='bg-black text-white p-2 border'>
                        Review {index + 1}
                      </AccordionButton>
                    </h3>
